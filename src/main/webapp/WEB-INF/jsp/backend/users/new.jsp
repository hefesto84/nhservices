...
<body>
	...
	<table class="table table-striped table-bordered table-hover table-custom">
		<tr>
			<th> Habitación </th>
			<td><input type="text" id="room" value=""></td>
		</tr>
		<tr>
			<th> Tablet </th>
			<td>
				<select id="tablet">
					<s:iterator value="tablets">
						<option value="<s:property value="id"/>"><s:property value="tablet" /></option>
					</s:iterator>
				</select>
			</td>
		</tr>
		...
		<tr>
			<th> IP Asterisk </th>
			<td><input type="text" id="ip_asterisk" value=""></td>
		</tr>
	</table>
	...
	<script type="text/javascript">
		...
		$('#boton_actualizar').click(function(){
			if($('#room').val() === ''){
				...
				var url_new = "<s:url value='/backend/users/add'/>";
				$.ajax({
					  method: "POST",
					  url: url_new,
					  data: {
